...
- **Null / missing keys** with an unexpected partitioner or producer setting that collapses load
...
1. **Confirm skew in size or lag, not just “the topic is slow”**
   - Compare **end offset / log size / lag per partition** for the topic.
...
2. Keep **ordering requirements** in mind: same key ⇒ same partition ⇒ ordered per key. If you need order per customer, customer id is fine *unless* one customer is enormous.
3. For a known hot entity (one huge tenant), see Fix C — do not hash everything on tenant id alone.
4. Roll out producer / Connect changes, then verify new traffic spreads (old data on the hot partition will drain on its own as it is consumed / expires).
...
3. Plan consumer group rebalance and any downstream systems that assume a fixed partition count.
...
- For salted / isolated keys: confirm the hot entity’s throughput and ordering meet the agreed trade-off
